...
---
* 11 Nov 2016: I think I have done some of these and forgotten to take them off the list... clean up the list!

* Make it a package

* On Specify tab, have the sentence about missingness indicators appear only if first option is chosen.

* 17 Feb 2017: Tom is getting an error in the sample size table: Class attributes at column 1 of input list at position 2 does not match...
...
* For list of vars to choose from on Prune tab, can I have the ones in the PS model show up first? I vaguely remember there's a problem with that

* Do they need to be 'imputation indicators' instead?

* The missing values are being re-imputed before each recalculation of the propensity score (they change when PSIDs() changes). Is this the behavior we want?

* Add exportable code for imputation and also for creation of imputation-indicator variables

* Possibly make the actual call to lrm() exportable (MS)
...
* Keep in mind: input$mainNavbarPage gives name of current tab.
